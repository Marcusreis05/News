...
app.post('/login', async (req, res)=>{
    ...
    if(!req.body.senha){
        return res.status(400).json({ message: 'O campo senha e obrigatorio'});
    }
    ...
    const token = jwt.sign({ _id: usuarioExistente._id}, 'reis');
    console.log(token)
...
app.listen(8080, () => {
    console.log('Sevitor funcionado na porta 8080')
});